...
.. code-block:: bash
  ...
  PROFILE="admin"
  LINE="credential_process = aws configure export-credentials --profile admin"

  # バックアップ
  \cp "$CONFIG" "$CONFIG.bak"

  # profile 存在確認
  if ! grep -q "^\[profile $PROFILE\]$" "$CONFIG"; then
    echo "profile $PROFILE が存在しません"
    exit 1
  fi

  # profile 内に credential_process があるか確認
  if awk -v profile="$PROFILE" '
    $0 == "[profile " profile "]" { in_profile=1; next }
    /^\[/ { in_profile=0 }
    in_profile && /^[[:space:]]*credential_process[[:space:]]*=/ {
      found=1
    }
    END { exit found ? 0 : 1 }
  ' "$CONFIG"; then
    echo "credential_process は既に存在します"
  else
    sed -i '' "/^\[profile $PROFILE\]/a\\
  $LINE
  " "$CONFIG"

    echo "credential_process を追加しました"
  fi
...
.. code-block:: bash
  ...
  key = "aws-transcribe-pii-reduction-by-glue/terraform.tfstate"
